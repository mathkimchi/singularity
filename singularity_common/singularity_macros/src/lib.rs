...
fn packet_union_impls(data_enum: syn::DataEnum) -> (proc_macro2::TokenStream, proc_macro2::TokenStream) {
    ...
    let try_from_data_impl = quote!{
        ...
        let id = PacketIdType::from_be_bytes(id_bytes.try_into().unwrap());
...
pub fn packet_derive(input: TokenStream) -> TokenStream {
    ...
    let packet_type_id = {
        ...
        // to do something like: `hash as singularity_common::sap::packet::PacketIdType`.
    ...
    quote! {
        const _: () = {
            ...
            impl PacketTrait for #identitifier {
                const PACKET_TYPE_ID: PacketIdType = #packet_type_id;
            }
        };
    }
    .into()
